...
export default function AddBot() {
  ...
  const [botModel, setBotModel] = useState('gemini-3.1-flash-lite');
  ...
  const handleSaveBot = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Bot baru berhasil dibuat!");
        router.push('/dashboard/bot');
      } else {
  ...
  return (
    <div className="flex flex-col w-full font-sans pb-10 space-y-8">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">Tambah Bot Baru</h2>
        </div>
      </div>
